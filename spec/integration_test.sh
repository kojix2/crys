...
printf 'Testing l processing options...\n'
...
run_cmd_with_stdin '1\n2\n3\n' "$BIN" --init 'sum = 0' -n 'sum += l.to_i' --final 'puts sum'
assert_status 0
assert_stdout_eq '6'

run_cmd "$BIN" --dump -p 'l.upcase'
assert_status 0
assert_stdout_contains '# generated by crys'
assert_stdout_contains 'l = begin'
assert_stdout_contains 'puts l'
...
printf 'foo\n' >"$TEST_DIR/a.txt"
printf 'bar\n' >"$TEST_DIR/b.txt"
run_cmd "$BIN" -n 'puts l' "$TEST_DIR/a.txt" "$TEST_DIR/b.txt"
assert_status 0
assert_stdout_eq $'foo\nbar'

run_cmd "$BIN" -n 'puts "#{path}:#{l}"' "$TEST_DIR/a.txt" "$TEST_DIR/b.txt"
assert_status 0
...
printf 'Testing fnr (per-file l number)...\n'
printf 'a\nb\n' >"$TEST_DIR/fnr2.txt"
...
run_cmd_with_stdin $'ok\nerror\nwarning\n' "$BIN" -n --where 'l =~ /error|warn/' 'puts l'
assert_status 0
assert_stdout_eq $'error\nwarning'
...
run_cmd_with_stdin $'1\nfoo\n3\nbar\n' "$BIN" --where 'l =~ /^[0-9]+$/' --sum 'l.to_i' --count
assert_status 0
